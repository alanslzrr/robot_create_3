...
Ejemplos de uso:
  ...
  python PRM01_P02.py --robot "MiRobot" --d-influence 50
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="ID de nodo de origen en el grafo (sobre-escribe q_i de points.json si se proporciona junto con --dest-node)",
    )
    parser.add_argument(
        "--dest-node",
        type=int,
        default=None,
        help="ID de nodo de destino en el grafo (sobre-escribe q_f y waypoints de points.json si se proporciona junto con --origin-node)",
    )
    parser.add_argument(
        "--potential",
    ...
    # Si se especifican nodos de origen y destino, usamos el grafo para
    # calcular el camino óptimo y generar dinámicamente points.json.
    if args.origin_node is not None and args.dest_node is not None:
        from grafos.prueba import cargar_grafo_desde_json

        print("\n" + "=" * 80)
        print("PLANIFICACIÓN DE RUTA EN GRAFO")
        print("=" * 80)
        try:
            graph = cargar_grafo_desde_json()
        except Exception as e:
            print(f"[ERROR] No se pudo cargar el grafo desde JSON: {e}")
            sys.exit(1)

        # Validar índices
        if not (0 <= args.origin_node < graph.V) or not (0 <= args.dest_node < graph.V):
            print(f"[ERROR] Los IDs de nodo deben estar entre 0 y {graph.V - 1}")
            sys.exit(1)

        print(f"  Origen [ID]: {args.origin_node} - {graph.nombres[args.origin_node]}")
        print(f"  Destino [ID]: {args.dest_node} - {graph.nombres[args.dest_node]}\n")

        # Obtener camino mínimo con Dijkstra (ya devuelve lista de índices y coste)
        path_indices, total_cost = graph.Camino_Minimo_Dijkstra(args.origin_node, args.dest_node)
        if not path_indices:
            print("[ERROR] No existe camino entre esos nodos en el grafo.")
            sys.exit(1)

        # Extraer coordenadas de cada nodo del camino
        coords = graph.coords
        for idx in path_indices:
            if idx >= len(coords) or coords[idx] is None:
                print(f"[ERROR] El nodo {idx} no tiene coordenadas definidas en el grafo.")
                sys.exit(1)

        # Construir q_i, waypoints y q_f a partir del camino
        first_idx = path_indices[0]
        last_idx = path_indices[-1]
        q_i_node = coords[first_idx]
        q_f_node = coords[last_idx]
        waypoint_indices = path_indices[1:-1]

        q_i = (q_i_node["x"], q_i_node["y"], q_i_node["theta"])
        waypoints = [(coords[i]["x"], coords[i]["y"]) for i in waypoint_indices]
        q_f = (q_f_node["x"], q_f_node["y"])

        # Guardar también en points.json para mantener compatibilidad con el resto
        points_path = Path(args.points)
        points_data = {
            "q_i": {"x": q_i[0], "y": q_i[1], "theta": q_i[2]},
            "q_f": {"x": q_f[0], "y": q_f[1]},
            "waypoints": [{"x": wp[0], "y": wp[1]} for wp in waypoints],
        }
        try:
            points_path.write_text(json.dumps(points_data, indent=2))
            print(f"\n[INFO] Ruta óptima escrita en '{points_path}'")
            print(f"       Camino (IDs): {' -> '.join(str(i) for i in path_indices)}")
            print(f"       Coste total: {total_cost:g}\n")
        except Exception as e:
            print(f"[WARNING] No se pudo escribir points.json: {e}")
    else:
        # Cargar los puntos de navegación desde el archivo JSON (modo clásico)
